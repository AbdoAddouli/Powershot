...
| **Weather API** (NOAA/AccuWeather) | Weather data for safety, production impact | Phase 4 |
| **Agentforce** (Einstein AI) | AI-powered field service, compliance, supply agents | Phase 4 |
| **Marketing Cloud** (Pardot) | Customer/partner journeys, automated campaigns | Phase 4 |
| **Salesforce Maps** | Geospatial well/lease/pipeline visualization | Phase 4 |
| **Field Service** (Dispatcher) | Mobile workforce, skills-based dispatch | Phase 4 |
| **Sustainability / ESG** | Carbon tracking, emissions reporting | Phase 5 |
| **Data Cloud** | Unify IoT, weather, production, ERP data | Phase 5 |
...
Phase 4 (6-12 months):
  Sales Cloud — O&G opportunity stages, Path, Kanban
  Service Cloud — Field Service territories, skills-based dispatch, mobile LWC
  Marketing Cloud — Journey Builder, automated campaigns, partner communications
  Agentforce — AI agents for field service, compliance, supply sales
  Einstein Next Best Action — contextual recommendations on record pages
  Einstein Bots — Partner self-service portal bot
  Weather API — integrate NOAA/AccuWeather for ops safety
  Salesforce Maps — geospatial visualization of wells, leases, pipelines
  E-signature — DocuSign for lease, JV, contract signing

Phase 5 (12-18 months):
  Sustainability & ESG — Carbon_Emission__c tracking, automated ESG reporting
  Data Cloud — unify SCADA, IoT, weather, ERP data with CRM
  Advanced IoT — real-time pipeline leak detection, tank monitoring via Platform Events
  AI-driven predictive maintenance — Einstein Prediction Builder on assets
  Automated regulatory filing — EPA CDX integration
  Executive KPI dashboard — cross-domain analytics with Data Cloud
```

---

### 10. Industry Clouds & Agentforce Strategy

#### 10.1 Salesforce Cloud Adoption Roadmap

| Cloud | Recommended | Key Use Cases | License Type |
|---|---|---|---|
| **Sales Cloud** | ✅ Yes | Opportunity stages (Supply Contract/Service Sale/JV Proposal), Path, Kanban, pipeline management | Included with Enterprise+ |
| **Service Cloud** | ✅ Yes | Field Service dispatch, work orders, service appointments, mobile LWC, skills-based routing | Field Service add-on license |
| **Marketing Cloud** | ✅ Recommended | Journey Builder for JV statements, regulatory reminders, safety bulletins, retail campaigns | Marketing Cloud Account Engagement |
| **Experience Cloud** | ✅ Yes (Phase 4) | O&G Partner Portal — JV partners, regulators, suppliers, field techs | Partner Community license |
| **Data Cloud** | ✅ Recommended | Unify SCADA, IoT, weather, ERP with CRM data for cross-domain insights | Data Cloud add-on |
| **Einstein AI** | ✅ Recommended | Prediction Builder (well decline, pipeline risk), Next Best Action, Bots | Einstein AI add-on |
| **Agentforce** | ✅ Recommended | AI agents for field service, compliance, supply sales | Agentforce add-on |

#### 10.2 Agentforce — Strategic Architecture

**Agents by Domain:**

| Agent | Audience | Key Topics | Channels |
|---|---|---|---|
| **Field Service Agent** | Field Technicians, Engineers | Well status, PTW validation, HSE reporting, inventory levels, inspection history | Salesforce Mobile, Record Page |
| **Supply Sales Agent** | Sales Team, Customers | Contract terms, pricing, delivery status, nominations | Sales Console, Partner Portal |
| **Compliance Agent** | Compliance Officers, Regulators | Permit expiry, report deadlines, filing requirements | Record Page, Portal |
| **Partner Support Bot** | JV Partners, Suppliers | Production data, royalty payments, delivery tracking, safety reporting | Experience Cloud Portal |

**Agentforce Data Security:**

| Data Type | Protection | Implementation |
|---|---|---|
| Pricing & Contract terms | Field-level masking | Einstein Trust Layer — Data Masking |
| HSE incident details | PII redaction | Einstein Trust Layer — PII Detection |
| JV financial data | Object-level access | Permission sets + Agentforce topic restrictions |
| IoT sensor readings | Read-only exposure | Agentforce action restrictions |

#### 10.3 Salesforce Maps & Geospatial

**Step 1: Enable Salesforce Maps**

```
Setup → Maps → Settings → Enable Salesforce Maps
  License: O&G Maps License (per user)
  Allowed Objects: Well__c, Pipeline__c, Terminal__c, Lease__c,
                   Land_Parcel__c, HSE_Incident__c, Retail_Outlet__c
```

**Step 2: Configure Map Layers**

| Layer | Data Source | Use Case |
|---|---|---|
| Well Locations | Well__c (Latitude/Longitude) | Well pad overview, production status color-coded |
| Pipeline Routes | Pipeline__c (Coordinate path) | Pipeline integrity, inspection status |
| Lease Boundaries | Land_Parcel__c (GIS polygon) | Lease area, mineral rights visualization |
| HSE Incidents | HSE_Incident__c (Location) | Safety incident heat map |
| Terminal/Refinery | Terminal__c, Refinery__c (Geo) | Asset map with inventory levels |
| Weather Overlay | Weather_Reading__c (Geo) | Real-time weather impact on operations |

**Step 3: Geospatial Queries**

```apex
// Find all wells within 5 miles of a pipeline incident
List<Well__c> nearbyWells = [SELECT Id, Name FROM Well__c
    WHERE DISTANCE(Location__c, :incidentGeo, 'mi') < 5];
```

---

### 11. Key Business Processes (MVP Scope)
